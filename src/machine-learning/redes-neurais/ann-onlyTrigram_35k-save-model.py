...
tflearn.config.init_graph(num_cores=0, gpu_memory_fraction=0)


model.load('./models/model_30k_OnlyTri')

yt = []
for i,sample in enumerate(url_test):
    yt.append(model.predict(sample))
    sys.stdout.write("\r Processing %d   " % i)
    sys.stdout.flush()
yt = np.array([yt])
np.save('./resultados/label_30k_OnlyTri.npy',np.array(label_test))
np.save('./resultados/saida_30k_OnlyTri.npy',yt)
